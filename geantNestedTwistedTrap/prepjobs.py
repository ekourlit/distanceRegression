...
def runGeantinoMap(macFName, nNest, outFName, workdir, basedir):
    start_time = time.time()
    process = subprocess.Popen(f'singularity exec /home/whopkins/fullsim-sources-geant4-v10.6.2.img /home/whopkins/distanceRegression/geantNestedTwistedTrap/build/wrapper.sh {workdir} {basedir} 1 2 {macFName} {nNest} {outFName}', stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
...
parser.add_argument('-w', '--workdir', default='/home/whopkins/distanceRegression/geantNestedTwistedTrap/build/', help='Working directory. This is where logs and outputs will be placed.')
...
for nNest in range(1,args.nNesting+1):
    ...
    writeTrainSubmit(submitFName, args.partition, jobName, nNodes, args.account, trainTime, args.cores, inputPath, args.workdir, args.trainBaseDir)
